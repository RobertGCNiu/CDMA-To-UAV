...
InterfPowMargin=0.5;
maxPathLossAllowed.dB=maxPtx.dBm-minPrx.dBm-InterfPowMargin;
maxUAVHeight=100;
...
while ~isempty(IndexRemainingUsers) && Iteration< maxIterations;    
    ...
    for ii=1:GridSearch.numCol
        for jj=1:GridSearch.numRow
            ...
            if currGridCenter.NumUsers
                ...
                % Compute Signal Power
                currGridCenter.SigPow=maxPtx.dBm-...
                    PathLossA2G(currGridCenter.Dist(currGridCenter.UserIndex),optUAVHeight);
                ...
                if DeployedUAV.totNum
                    ...
                    for kk=1:DeployedUAV.totNum
                        ...
                        InterPowerdB(kk,:)=maxPtx.dBm-PathLossA2G(currInterfDist,DeployedUAV.Height(kk));
                    end
                end
                ...
                if (~sum(currGridCenter.UserSINR < minSINR.dB))
                    ...
                    if (isempty(IndexAssignedUsers) || ~sum(currGridCenter.SINRAssignedUsers < minSINR.dB))
    ...
    if maxNumNewUserCovered > 0
        ...
        DeployedUAV.Height(DeployedUAV.totNum,1)=optUAVHeight;
...
%         pause;
        ...
        if(rand<0.5)
            maxUAVHeight=maxUAVHeight/3*2;
        else
            maxPtx.dBm=maxPtx.dBm-1;
            maxPathLossAllowed.dB=maxPtx.dBm-minPrx.dBm-InterfPowMargin;
        end
        [Rmax,optUAVHeight] = FindMaxR(maxPathLossAllowed.dB,maxUAVHeight);
    end
