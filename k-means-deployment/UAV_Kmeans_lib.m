...
users_sum = 0;
%%%%%%%%%%%%%%%%%%%%%%
%--------Cell breathing to constrain the number of users. I have close this
%function.
for k = 1:length(k_means_center)
    users_in_cluster = [];
    for k_users = 1: length(users_dist)
        r_distance(k_users,k) = norm(users_dist(k_users,:)-k_means_center(k,1:2));
       if r_distance(k_users,k)<k_means_center(k,3)
           users_in_cluster = [users_in_cluster;r_distance(k_users,k), users_dist(k_users,:)];
       end
    end
    %--------------cell breathing
...
if R_cov>0.8 && sum(k_means_center(:,3)<R_m)==k_cluster
...
% all_area = sum(pi*k_means_center(:,3).^2);
% rate_overlap = all_overlap_area/all_area;
% if rate_overlap < 0.1 && sum(k_means_center(:,3)<R_m)==k_cluster
%     for k_circle = 1 : k_cluster
%          circle(k_means_center(k_circle,1),k_means_center(k_circle,2),k_means_center(k_circle,3));
%          plot(k_means_center(k_circle,1),k_means_center(k_circle,2),'*','MarkerSize',16)
%     end
